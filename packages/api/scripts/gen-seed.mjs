...
// Real 2026 World Cup Round-of-32 kickoffs, keyed by the bracket slot id. The slots
// follow the official template (R32-1..R32-16 = matches 73..88), so each has a fixed
...
// match schedule (cross-checked against ESPN/Yahoo/Sky day-by-day listings).
const R32_KICKOFFS = {
...
// R32 now uses the exact per-slot times above; the later rounds still spread their
// window evenly (real teams/dates fill in as the bracket resolves).
const KO_DATES = {
...
for (const [stage, count] of KO) {
  ...
  for (let i = 1; i <= count; i++) {
    const id = `${stage}-${i}`
    // R32 has a real per-slot kickoff; later stages spread evenly across their window.
    const kickoff = R32_KICKOFFS[id]
      ? etToUtc(R32_KICKOFFS[id])
      : window.length ? window[Math.floor(((i - 1) * window.length) / count)] : null
    lines.push(
      `INSERT INTO matches (id,stage,grp,home_team_id,away_team_id,kickoff,status) VALUES (` +
